...
void main() {
    ...
    if (all(lessThan(texelPos, uval_mainImageSizeI))) {
        ...
        if (viewZ > -65536.0) {
            gbufferData1_unpack(texelFetch(usam_gbufferSolidData1, coords_renderTexelToViewTexel(texelPos), 0), lighting_gData);
            gbufferData2_unpack(texelFetch(usam_gbufferSolidData2, coords_renderTexelToViewTexel(texelPos), 0), lighting_gData);
