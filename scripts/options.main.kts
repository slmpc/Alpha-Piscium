...
Lanczos2：与Catmull-Rom一样清晰，但振铃或光晕较少。性能开销最大。"""
                ...
                slider("SETTING_RENDER_SCALE", 1.0, 0.5..1.0 step 0.05) {
                    lang {
                        name = "Render Resolution Scale"
                        comment =
                            "Renders the main lighting and post-processing pipeline at a lower internal resolution, then upscales to the window. Lower values improve performance but reduce image detail."
                        prefix = "x"
                    }
                    lang(Locale.SIMPLIFIED_CHINESE) {
                        name = "渲染分辨率比例"
                        comment =
                            "以较低内部解析度渲染主要光照和后处理管线，然后升采样到窗口分辨率。数值越低性能越好，但图像细节会减少。"
                        prefix = "x"
                    }
                }
                slider("SETTING_RENDER_SCALE_SHARPNESS", 0.5, 0.0..1.0 step 0.05) {
                    lang {
                        name = "Upscale Sharpness"
                        comment =
                            "Controls the Catmull-Rom reconstruction sharpness used when render resolution scale is below native."
                    }
                    lang(Locale.SIMPLIFIED_CHINESE) {
                        name = "升采样锐度"
                        comment = "控制渲染分辨率低于原生时 Catmull-Rom 重建滤镜的锐度。"
                    }
                }
